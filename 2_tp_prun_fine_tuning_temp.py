...
def main():
    ...
    # 3.0 importance criterion for parameter selections
    ...
    # pruner = tp.pruner.MagnitudePruner(sparse_model, example_inputs,
    ...
    #
    #
    # # 22. load target pretrained model
    # check_path = './checkpoints/model_best_resnet18_prun_loc_0.2.pth.tar'
    # load_checkpoint(check_path, sparse_model, device)
    # test_acc1 = test(val_loader, sparse_model, device, class_to_idx, classes, class_acc=False, print_freq=10)
    # print(f" Acc target model pruning : {test_acc1}")
    # print(f"model: {sparse_model}")  # print model structure

    new_check_path = './checkpoints/resnet18_prun_loc_0.2.pth.tar'
    #torch.save(sparse_model, new_check_path)
    sparse_model = torch.load(new_check_path, map_location=device)
    test_acc12 = test(val_loader, sparse_model, device, class_to_idx, classes, class_acc=False, print_freq=10)
    print(f" Acc target model pruning : {test_acc12}")
    print(f"model: {sparse_model}")  # print model structure
    ...
    # 4 finetune model
    if global_pruning:
        save_name = f'resnet18_prun_group_0.2_{str(ch_sparsity)}'
    else:
        save_name = f'resnet18_prun_loc_0.2_{str(ch_sparsity)}'
    print(f'==> {save_name}')
